...
def resize_images(images, labels, target_size=(256, 256)):
    resized_images = []
    for image in images:
...
def crop_images_and_labels(images, labels, min_crop_margin=0.05, max_crop_margin=0.25, random_seed=None):
    ...
    for image, label in zip(images, labels):
        ...
        if new_w <= 0 or new_h <= 0:
            print("Invalid crop margin leading to non-positive dimensions. Skipping this image.")
            continue
...
images, labels = read_and_normalize_data(image_folder, label_folder,target_size=(600,800))
...
all_image2, all_labels2 = resize_images(all_images, all_labels, target_size=(256, 256))


all_labels2 = normalize_labels(all_labels2)



X_train, X_test, y_train, y_test = train_test_split(all_image2, all_labels2, test_size=0.2, random_state=42)
...
model_path = '/Users/hossein/Desktop/CV/PROJECTS/yolo_model2.h5'
